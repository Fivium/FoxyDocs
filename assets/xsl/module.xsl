...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fm="http://www.og.dti.gov/fox_module">
  ...
  <xsl:template match="/xs:schema/xs:annotation/xs:appinfo/fm:module">
    ...
    <html>
      <head>
        <title>
          <xsl:value-of select="fm:header/fm:name"/> - Module Specification</title>
      ...
      <body>
        <div class="header">
          ...
          <div class="nav">
            <h1>
              <xsl:value-of select="fm:header/fm:application-title"/> - <xsl:value-of select="fm:header/fm:title"/> (<xsl:value-of select="fm:header/fm:name"/>)
            </h1>
          </div>
        </div>
        <p>
          ...
          <xsl:otherwise>
            <xsl:for-each select="//fm:call-module">
              ...
              <xsl:choose>
                <xsl:when test="$MODULE_LIST/MODULE[text() = $MODULE_NAME]">
                  <xsl:element name="a">
                    <xsl:attribute name="href"><xsl:value-of select="@module"/>.html</xsl:attribute>
                    <xsl:value-of select="@module"/>
                  </xsl:element>
                  -
                  <xsl:element name="a">
                    <xsl:attribute name="href"><xsl:value-of select="@module"/>.html#entry-theme-<xsl:value-of select="@theme"/></xsl:attribute>
                    <xsl:value-of select="@theme"/>
                  </xsl:element>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="@module"/> - <xsl:value-of select="@theme"/>
                </xsl:otherwise>
              </xsl:choose>
              <br/>
            </xsl:for-each>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:for-each select="fm:entry-theme-list/fm:entry-theme">
              <xsl:element name="a">
                <xsl:attribute name="name">entry-theme-<xsl:value-of select="@name"/></xsl:attribute>
              </xsl:element>
              <h2>
                <xsl:value-of select="@name"/>
              </h2>
                    ...
                    <ul>
                      <xsl:for-each select="fm:param-list/fm:param">
                        <li>
                          <xsl:choose>
                            <xsl:when test="@mand='.'">
                              <b><xsl:value-of select="@xpath"/> (<xsl:value-of select="@type"/>)</b>
                            </xsl:when>
                            <xsl:otherwise>
                              <xsl:value-of select="@xpath"/> (<xsl:value-of select="@type"/>)
                            </xsl:otherwise>
                          </xsl:choose>
                        </li>
                      </xsl:for-each>
                  ...
                  <xsl:otherwise>
                    <ul>
                      <xsl:for-each select="fm:return-list/fm:return">
                        <li>
                          <xsl:value-of select="@xpath"/> (<xsl:value-of select="@type"/>) <xsl:choose>
                            <xsl:when test="@mand='.'">Required</xsl:when>
